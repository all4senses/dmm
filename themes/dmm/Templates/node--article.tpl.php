...
<?php endif; ?>
            ...
            <?php 
              ...
              if ($node->type != 'section') {
                  ...
                  if ($page) {
                    ...
                    if (isset($author->field_social_profiles['und'])) {
                      foreach ($author->field_social_profiles['und'] as $value) {
      ...
      <div class="content <?php echo ($page ? 'page' : 'teaser'); ?>"<?php print $content_attributes; ?>>
        <?php
          ...
          if (isset($content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) && $content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) {
            dmm_misc_addMetatag($keyword_metatag_name, $content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']);
          }
          elseif (@$content['field_tags']) {
            dmm_misc_pushTagsToMetatags($keyword_metatag_name, $content['field_tags']);
          }
      ...
      <?php 
        if (!$user->uid) {
          ...
          echo '<div class="login-to-comment"><a class="ctools-use-modal ctools-modal-modal-popup-medium-dmm" href="/modal_forms/nojs/login?destination=http://dommoejmechty.com.ua' . url('node/' . $node->nid, array('absolute' => TRUE, 'fragment' => 'comment-form')) . '">Modal Login</a></div>';
