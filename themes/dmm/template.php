 ...
 */
function dmm_html_head_alter(&$head_elements) {
 ...
 */
function dmm_breadcrumb($variables) {
 ...
 */
function dmm_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function dmm_process_page(&$variables) {
 ...
 */
function dmm_preprocess_node(&$variables) {
 ...
 */
function dmm_preprocess_region(&$variables, $hook) {
 ...
 */
function dmm_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function dmm_process_block(&$variables, $hook) {
 ...
 */
function dmm_preprocess_search_block_form(&$vars) {
 ...
 */
function dmm_form_alter(&$form, &$form_state, $form_id) {
...
*/
function dmm_preprocess_views_view_rss(&$vars) {
...
* Default theme function for all RSS rows.
*/
function dmm_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = dmm_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
    ...
    $rss_teaser = preg_replace('|href="/|', 'href="http://dommoejmechty.com.ua/', $rss_teaser);
  ...
  $vars['item_elements'] = preg_replace('|<dc:creator>.*</dc:creator>|', '<dc:creator>' . dmm_misc_getUserRealName($node->uid) . '</dc:creator>', $vars['item_elements']);
}
 ...
 */
/*
function dmm_preprocess_views_view_unformatted(&$vars) {
 ...
 */
/*
function dmm_captcha($variables) {
 ...
 */
/*
function dmm_preprocess_views_view(&$vars) {
  
  // I set title for preface (at dmm_misc_views_pre_render(&$view)) instead of a view itself.
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'dmm_preprocess_views_view_unformatted'; 
...
/*
function dmm_preprocess_html(&$variables) {
  //dpm($variables);
  //global $user;
  //if ($user->uid == 1) {
